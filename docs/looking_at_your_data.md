...
Jupyter работает на языке Python, поэтому некоторые команды, которые вы будете выполнять в этих упражнениях с базой данных, потребуют включения небольшого количества кода Python вместе с самим SQL-запросом.
