...
Обратите внимание, что вам необходимо использовать набор функций, отличный от того, который вы использовали бы для обычных числовых данных, чтобы добавить или вычесть время из любых значений в этих форматах даты и времени.  Например, вместо того, чтобы использовать знак минус для определения разницы во времени между двумя временными метками или датами, вы могли бы использовать функцию TIMEDIFF или DATEDIFF.

<!-- TODO Дополнить про YEAR и DATE -->


### Вопрос 4

**Какой запрос необходимо написать, чтобы получить все идентификаторы собак `dog_guid` и время последнего обновления информации `updated_at`  из таблицы тестов `complete_tests`, завершенных до 15 октября 2015 г.**
...
Напишите запрос, который покажет все идентификаторы пользователей `user_guid` из таблицы `dogs`, которые являются владельцами собак женского пола (значение _female_ из колонки `gender`) и порода включает в себя слово _terrier_ (колонка `breed`).
